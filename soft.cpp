...
public:
    ...
    unordered_map<int, vector<float>> prob_distribution_map;

    // Store the lines and positions of question marks.
    vector<pair<int, int>> missing_values_positions;

    // Constructor
    vector<vector<vector<float>>> counts; // counts[i][k][j] is the number of times variable i  with parents taking kth configuration takes value j
    Dataset() {}
    ...
    void add_record(const vector<string>& record) {
        ...
        for (int i = 0; i < record.size(); ++i) {
            ...
            if (value.substr(1,value.size()-2) == "?") {
                ...
                // for row i in probablity distribution map, set the value of column i to vector of length of values of variable i
                prob_distribution_map[i] = vector<float>(variable_value_map[i].size(), 0.0);
            } else {
    ...
    bool check_parents_missing(vector<int> records, vector<int> parent_pos){
        for (int i = 0; i < parent_pos.size(); ++i) {
            if (records[parent_pos[i]] == -1) {
                return true;
            }
        }
        return false;
    ...
    void set_counts(network& Alarm) {
		...
		for (int j = 0; j < data_matrix[0].size(); j++) {
			...
			// cout <<endl ; 
			counts[j].resize(multiply_further, vector<float>(node.get_values().size() , 0.0));
        ...
        for (int i = 0; i < data_matrix.size(); ++i) {
            ...
            for (int j = 0; j < record.size(); j++) {
                ...
                // three cases 
                // 1. node is -1
                // 2. node is not -1 and one of parents is -1
                // 3. node is not -1 and none of parents is -1
                ...
                if (record[j] == -1){
                    ...
                    for (int k = 0; k < node.get_values().size(); ++k) {
                        counts[j][pos][k] += prob_distribution_map[i][k];
                    }
                    continue;
                }
                else if (check_parents_missing(record, parents_pos)){
                    // cout << "case 2" << endl;
                    for (int k = 0; k < parents_pos.size(); ++k) {
                    ...
                    // make a vector of possible values of parents with -1 replaced by a possible value of that parent
                    vector<vector<int>> possible_parents_values;
                    for (int k = 0; k < parents_pos.size(); ++k) {
                        vector<int> temp_parents_values = parents_values;
                        if (temp_parents_values[k] == -1) {
                            for (int l = 0; l < Alarm.Pres_Graph[parents_pos[k]].get_values().size(); ++l) {
                                temp_parents_values[k] = l;
                                possible_parents_values.push_back(temp_parents_values);
                            }
                        }
                    }
                    // for each possible value of parents, add the probablity of the node taking the value record[j]
                    for (int k = 0; k < possible_parents_values.size(); ++k) {
                        int pos = get_parent_config(possible_values, possible_parents_values[k]);
                        counts[j][pos][record[j]] += prob_distribution_map[i][k];
                    }
                    continue;
                }
                else{
...
std::vector<float> generateRandomVectorWithSumOne(int size) {
    if (size <= 0) {
        return {};
    }
    ...
    // Set the last element such that the sum of the vector is 1
    vec[size - 1] = 1 - sum;
...
void random_initialise_data(Dataset& dataset1, Dataset& dataset2, network& Alarm) {
    ...
    for (const auto& position : missing_positions) {
        ...
        // make a vector of probability of size possibleValues.size() and its sum = 1
        vector<float> probablity_dist(possibleValues.size(), 0.0);
        dataset2.prob_distribution_map[row] = generateRandomVectorWithSumOne(possibleValues.size());
    }
}
...
void evaluate_CPT(network& Alarm, Dataset& dataset) {
    for (int i = 0; i < Alarm.netSize(); ++i) {
        ...
        vector<vector<float>> counts = dataset.counts[i];
...
float prob_to_be_i_given_parents_probability_dist(Graph_Node node, vector<float> probablity_dist,vector<int> record,network& Alarm, int to_be_i){
    ...
    for (int k = 0; k < possible_parents_values.size(); ++k) {
        int pos = get_parent_config(possible_values, possible_parents_values[k]);
        prob += probablity_dist[k]*node.get_CPT()[pos];
    }
    return prob;
}
    ...
    for (const auto& missingValue : dataset1.get_missing_values_positions()) {
        ...
        rep(i, 0 , node.get_nvalues()){
            ...
            for(int child : node.Children){
                ...
                record[col] = i;
                prob_tobe_i*= prob_to_be_i_given_parents_probability_dist(child_node,dataset2.prob_distribution_map[row] ,record ,Alarm,record[child] );
            }
        ...
        dataset2.prob_distribution_map[row] = probablity_dist;
        // cout  <<5 << endl;
        missingCounter++;
